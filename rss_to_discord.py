...
def summarize_article(title, description, groq_api_key):
    ...
    prompt = (
        f"You write concise technical summaries for a Discord cybersecurity channel. "
        f"Write 2-3 sentences in {lang}.\n\n"
        f"Rules:\n"
        f"- First sentence: the key technical finding or vulnerability (what was found, how it works)\n"
        f"- Second sentence: real-world impact, affected systems, scope (CVE numbers, software names, versions, number of affected users if available)\n"
        f"- Third sentence (optional): why it matters or what makes it interesting, to make the reader want to dig deeper\n"
        f"- Be technically precise: mention specific attack techniques, protocols, CVEs, tools\n"
        f"- Stay factual, do NOT invent CVE numbers or stats that are not in the content\n"
        f"- Use a direct tone, no filler words\n"
